...
class CountryStorageTest {
    ...
    // Note: The test for IOException handling during initialization was removed
    // because it relied on reflection to modify final static fields, which is not
    // allowed in Java 17 due to stronger encapsulation. The IOException handling
    // in CountryStorage is still present and functional, but cannot be unit tested
    // without refactoring the class to use dependency injection for the ObjectMapper.
